...
public class StoreWorld extends World {
	...
	public WorldImage createDisp() {
		...
		if (selections.getSelectedNum() < Config.itemNum) {
			...
			if (User.nowItemLevels[selections.getSelectedNum()] < Config.items[selections.getSelectedNum()].maxLevel) {
				color = Color.black;
			}
			else color = Color.red;
			pic = new OverlayImages(pic, new TextImage(new Posn(100, 525), "NOW LEVEL:"+User.nowItemLevels[selections.getSelectedNum()], 25, color));
    ...
    public World onKeyEvent(String k) {
        ...
        if (k.equals("z")) {
        	if (selections.getSelectedNum() == selectionN * selectionM - 1) {
        		...
        		if (selections.getSelectedNum() >= Config.itemNum) return this;
